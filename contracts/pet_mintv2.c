...
int64_t hook(uint32_t reserved)
{
    ...
    int64_t mint_price = float_int(mint_price_xfl, 6, 1);
    if (mint_price != payment)
        rollback(SBUF("pet_mint.c: Insufficient Payment `Amount`"), __LINE__);
...
#define ACCT_OUT (seed_data + 0U)
    ...
    if (otxn_param(SBUF(hn_buff), SBUF(hm_key)) != 14)
    {
